...
public class StringUtils {
	...
	/*
	 * public static void main(String[] args) { SimpleDateFormat df = new
	 * SimpleDateFormat("yyyy-MM-dd");// 设置日期格式 String
	 * mondayString=StringUtils.getMonday(df.format(new Date()))+ " 00:00:00";
	 * String sumdayString=StringUtils.getSunday(df.format(new Date()))+
	 * " 23:59:59"; String monday=DateUtils.getTime(mondayString)+""; String
	 * sumday=DateUtils.getTime(sumdayString)+""; //System.out.println(monday);
	...
	/*
	 * public static String fillTemplet(String template){ //V1.6.6.1 RDPROJECT-331
	 * liukun 2013-10-12 start //模板中的'是非法字符，会导致无法提交，所以页面上用`代替 template =
	 * template.replace('`', '\''); //V1.6.6.1 RDPROJECT-331 liukun 2013-10-12 end
	 * 
	 * Map<String,Object> data=Global.getTransfer(); try { return
	 * FreemarkerUtil.renderTemplate(template, data); } catch (Exception e) {
	...
	public static int getSexByCardid(String cardId) {
		/*
		 * String sexNum = ""; if (cardId.length() == 15) { sexNum =
		 * cardId.substring(13, 14); } else { sexNum = cardId.substring(16, 17); }
		 * 
		 * if (Integer.parseInt(sexNum) % 2 == 1) { return 1; } else { return 0; }
		 */
		int sexNum = 0;
		if (cardId.length() == 15) {
